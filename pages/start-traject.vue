    ...
    <header class="fixed top-0 w-full z-50 bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-sm">
      <div class="mx-auto max-w-7xl px-6 sm:px-8 lg:px-12 py-6 flex items-center justify-between">
        ...
        <nav class="hidden md:flex items-center gap-8 text-sm font-medium">
          ...
          <nuxt-link to="/klanten" class="text-gray-700 hover:text-emerald-600 transition-colors">{{ t('customers.nav') }}</nuxt-link>
          <nuxt-link to="/partners" class="text-gray-700 hover:text-emerald-600 transition-colors">{{ t('partner.nav') }}</nuxt-link>
        </nav>
          ...
          <form action="https://api.web3forms.com/submit" method="POST" class="p-8 space-y-6">
            ...
            <div>
              ...
              >
                ...
                <option value="CO2-Prestatieladder">{{ currentLocale === 'nl' ? 'CO2-Prestatieladder' : 'CO2 Performance Ladder' }}</option>
                <option value="Anders">{{ currentLocale === 'nl' ? 'Anders' : 'Other' }}</option>
              </select>
            </div>
