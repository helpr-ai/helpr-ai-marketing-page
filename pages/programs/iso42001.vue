<template>
  <div class="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-violet-50">
    <!-- Header/Nav -->
    <header class="fixed top-0 w-full z-50 bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-sm">
        ...
        <div class="hidden md:flex items-center gap-4">
          <div class="relative">
            ...
            <div v-if="showLangDropdown" class="absolute right-0 top-full mt-2 bg-white rounded-xl shadow-xl border border-gray-200 py-2 min-w-[160px] z-50">
              <NuxtLink v-for="loc in availableLocales" :key="loc.code" :to="switchLocalePath(loc.code)" :class="['w-full flex items-center gap-3 px-4 py-2 text-sm hover:bg-gray-50 transition-colors', locale === loc.code ? 'bg-indigo-50 text-indigo-600' : 'text-gray-700']">
                <span class="text-lg">{{ loc.flag }}</span>
                <span>{{ loc.name }}</span>
              </NuxtLink>
            </div>
          </div>
          <NuxtLink :to="localePath('/') + '#demo'" class="h-11 flex items-center rounded-xl px-6 bg-gradient-to-r from-indigo-600 to-violet-600 text-white hover:from-indigo-700 hover:to-violet-700 transition-all shadow-lg hover:shadow-xl font-medium">{{ $t('nav.demo') }}</NuxtLink>
        </div>
      ...
      <div v-if="showMobileMenu" class="md:hidden absolute top-full left-0 right-0 bg-white border-b border-gray-200 shadow-lg z-40">
        <div class="px-6 py-4 space-y-4">
          ...
          <div class="pt-2 pb-4">
            <NuxtLink :to="localePath('/') + '#demo'" @click="closeMobileMenu()" class="block w-full h-11 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 text-white font-medium text-center leading-[44px]">{{ $t('nav.demo') }}</NuxtLink>
          </div>
        </div>
      </div>
    </header>
    ...
    <!-- Hero Section -->
    <section class="relative overflow-hidden pt-36 pb-20 bg-gradient-to-br from-indigo-600 via-violet-600 to-indigo-800">
      <div class="absolute inset-0 bg-gradient-to-r from-indigo-500/10 to-violet-500/10"></div>
      <div class="absolute -top-24 -left-24 w-96 h-96 bg-gradient-to-r from-indigo-400/20 to-violet-400/20 rounded-full blur-3xl"></div>
      <div class="absolute -bottom-24 -right-24 w-96 h-96 bg-gradient-to-r from-violet-400/20 to-indigo-400/20 rounded-full blur-3xl"></div>

      <div class="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="text-center max-w-4xl mx-auto">
          <NuxtLink :to="localePath('programs')" class="inline-flex items-center text-indigo-100 hover:text-white mb-6 transition-colors">
          ...
          <p class="mt-4 text-xl text-indigo-100">{{ $t('programs.iso42001.tagline') }}</p>
          <p class="mt-8 text-xl sm:text-2xl text-indigo-100 leading-relaxed max-w-3xl mx-auto">
            {{ $t('iso42001Coming.hero.description') }}
          </p>
        </div>
      </div>
    </section>
    ...
    <section class="py-24">
      <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="grid lg:grid-cols-2 gap-12 items-center">
          <div>
            <h2 class="text-3xl sm:text-4xl font-bold tracking-tight text-gray-900 mb-6">{{ $t('iso42001Coming.about.title') }}</h2>
            <p class="text-xl text-gray-600 leading-8 mb-8">{{ $t('iso42001Coming.about.description') }}</p>
            <div class="space-y-4">
              <div v-for="(point, index) in tm('iso42001Coming.about.points')" :key="index" class="flex items-start gap-3">
                <CheckCircleIcon class="h-6 w-6 text-indigo-600 mt-0.5 flex-shrink-0" />
                <span class="text-gray-700">{{ rt(point) }}</span>
              </div>
            </div>
          </div>
          <div class="bg-gradient-to-br from-indigo-500 to-violet-600 rounded-3xl p-8 lg:p-12 text-white">
            <div class="w-16 h-16 bg-white/20 rounded-2xl flex items-center justify-center mb-6">
              <CpuChipIcon class="h-8 w-8 text-white" />
            </div>
            <h3 class="text-2xl font-bold mb-4">{{ $t('iso42001Coming.about.helprTitle') }}</h3>
            <p class="text-indigo-100 leading-7">{{ $t('iso42001Coming.about.helprDescription') }}</p>
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="py-24 bg-gray-50">
      <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 text-center">
        <div class="bg-white rounded-3xl shadow-xl border border-gray-200 p-8 lg:p-12">
          ...
          <NuxtLink :to="localePath('/') + '#demo'" class="inline-flex items-center justify-center rounded-2xl bg-gradient-to-r from-indigo-600 to-violet-600 px-8 py-4 text-lg font-semibold text-white shadow-xl hover:from-indigo-700 hover:to-violet-700 transition-all duration-200 hover:scale-[1.02]">
...
useHead({
  title: computed(() => locale.value === 'nl' ? 'ISO 42001 (Binnenkort) | Helpr.AI' : 'ISO 42001 (Coming Soon) | Helpr.AI'),
  meta: [{ name: 'description', content: computed(() => locale.value === 'nl'
    ? 'ISO 42001 AI governance certificering komt binnenkort naar Helpr.ai. Meld je aan voor early access.'
    : 'ISO 42001 AI governance certification is coming soon to Helpr.ai. Sign up for early access.') }]
})
</script>
