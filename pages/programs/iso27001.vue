<template>
  <div class="min-h-screen bg-gradient-to-br from-purple-50 via-white to-indigo-50">
    ...
    <header class="fixed top-0 w-full z-50 bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-sm">
      <div class="mx-auto max-w-7xl px-6 sm:px-8 lg:px-12 py-6 flex items-center justify-between">
        ...
        <nav class="hidden md:flex items-center gap-8 text-sm font-medium">
          ...
          <NuxtLink :to="localePath('programs')" class="text-purple-600 font-semibold">{{ $t('programs.nav') }}</NuxtLink>
        ...
        <div class="hidden md:flex items-center gap-4">
          <div class="relative">
            ...
            <div v-if="showLangDropdown" class="absolute right-0 top-full mt-2 bg-white rounded-xl shadow-xl border border-gray-200 py-2 min-w-[160px] z-50">
              <NuxtLink v-for="loc in availableLocales" :key="loc.code" :to="switchLocalePath(loc.code)" @click="showLangDropdown = false" :class="['w-full flex items-center gap-3 px-4 py-2 text-sm hover:bg-gray-50 transition-colors', locale === loc.code ? 'bg-purple-50 text-purple-600' : 'text-gray-700']">
                <span class="text-lg">{{ loc.flag }}</span>
                <span>{{ loc.name }}</span>
              </NuxtLink>
            </div>
          </div>
          <NuxtLink :to="localePath('/') + '#demo'" class="h-11 flex items-center rounded-xl px-6 bg-gradient-to-r from-purple-600 to-indigo-600 text-white hover:from-purple-700 hover:to-indigo-700 transition-all shadow-lg hover:shadow-xl font-medium">{{ $t('nav.demo') }}</NuxtLink>
        </div>
      ...
      <div v-if="showMobileMenu" class="md:hidden absolute top-full left-0 right-0 bg-white border-b border-gray-200 shadow-lg z-40">
        <div class="px-6 py-4 space-y-4">
          <div class="space-y-2">
            ...
            <NuxtLink :to="localePath('programs')" @click="closeMobileMenu()" class="block w-full text-left px-3 py-2 text-purple-600 font-semibold bg-purple-50 rounded-lg">{{ $t('programs.nav') }}</NuxtLink>
          ...
          <div class="pt-2 pb-4">
            <NuxtLink :to="localePath('/') + '#demo'" @click="closeMobileMenu()" class="block w-full h-11 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-medium text-center leading-[44px]">{{ $t('nav.demo') }}</NuxtLink>
          </div>
        </div>
      </div>
    </header>
    ...
    <!-- Hero Section -->
    <section class="relative overflow-hidden pt-36 pb-20 bg-gradient-to-br from-purple-600 via-indigo-600 to-purple-800">
      <div class="absolute inset-0 bg-gradient-to-r from-purple-500/10 to-indigo-500/10"></div>
      ...
      <div class="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="text-center max-w-4xl mx-auto">
          <NuxtLink :to="localePath('programs')" class="inline-flex items-center text-purple-100 hover:text-white mb-6 transition-colors">
          ...
          <p class="mt-4 text-xl text-purple-100">{{ $t('programs.iso27001.tagline') }}</p>
          <p class="mt-8 text-xl sm:text-2xl text-purple-100 leading-relaxed max-w-3xl mx-auto">
            {{ $t('iso27001Coming.hero.description') }}
          </p>
        </div>
      </div>
    </section>
    ...
    <section class="py-24">
      <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="grid lg:grid-cols-2 gap-12 items-center">
          <div>
            <h2 class="text-3xl sm:text-4xl font-bold tracking-tight text-gray-900 mb-6">{{ $t('iso27001Coming.about.title') }}</h2>
            <p class="text-xl text-gray-600 leading-8 mb-8">{{ $t('iso27001Coming.about.description') }}</p>
            <div class="space-y-4">
              <div v-for="(point, index) in tm('iso27001Coming.about.points')" :key="index" class="flex items-start gap-3">
                <CheckCircleIcon class="h-6 w-6 text-purple-600 mt-0.5 flex-shrink-0" />
                <span class="text-gray-700">{{ rt(point) }}</span>
              </div>
            </div>
          </div>
          <div class="bg-gradient-to-br from-purple-500 to-indigo-600 rounded-3xl p-8 lg:p-12 text-white">
            <div class="w-16 h-16 bg-white/20 rounded-2xl flex items-center justify-center mb-6">
              <LockClosedIcon class="h-8 w-8 text-white" />
            </div>
            <h3 class="text-2xl font-bold mb-4">{{ $t('iso27001Coming.about.helprTitle') }}</h3>
            <p class="text-purple-100 leading-7">{{ $t('iso27001Coming.about.helprDescription') }}</p>
          </div>
        </div>
      </div>
    </section>
      ...
      <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 text-center">
        <div class="bg-white rounded-3xl shadow-xl border border-gray-200 p-8 lg:p-12">
          <div class="w-16 h-16 bg-purple-100 rounded-2xl flex items-center justify-center mx-auto mb-6">
            <BellIcon class="h-8 w-8 text-purple-600" />
          </div>
          ...
          <NuxtLink :to="localePath('/') + '#demo'" class="inline-flex items-center justify-center rounded-2xl bg-gradient-to-r from-purple-600 to-indigo-600 px-8 py-4 text-lg font-semibold text-white shadow-xl hover:from-purple-700 hover:to-indigo-700 transition-all duration-200 hover:scale-[1.02]">
...
useHead({
  title: computed(() => locale.value === 'nl' ? 'ISO 27001 (Binnenkort) | Helpr.AI' : 'ISO 27001 (Coming Soon) | Helpr.AI'),
  meta: [{ name: 'description', content: computed(() => locale.value === 'nl'
    ? 'ISO 27001 informatiebeveiliging certificering komt binnenkort naar Helpr.ai. Meld je aan voor early access.'
    : 'ISO 27001 information security certification is coming soon to Helpr.ai. Sign up for early access.') }]
})
</script>
