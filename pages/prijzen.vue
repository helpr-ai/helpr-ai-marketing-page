<template>
  <div class="min-h-screen bg-gradient-to-br from-slate-50 via-white to-indigo-50">
    ...
    <header class="fixed top-0 w-full z-50 bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-sm">
      <div class="mx-auto max-w-7xl px-6 sm:px-8 lg:px-12 py-6 flex items-center justify-between">
        ...
        <div class="hidden md:flex items-center gap-4">
          ...
          <button @click="scrollTo('cta')" class="h-11 flex items-center rounded-xl px-6 bg-gradient-to-r from-indigo-600 to-purple-600 text-white hover:from-indigo-700 hover:to-purple-700 transition-all shadow-lg hover:shadow-xl font-medium">{{ $t('pricing.hero.cta') }}</button>
        </div>
    ...
    <section class="relative overflow-hidden pt-36 pb-24 bg-gradient-to-br from-indigo-900 via-purple-900 to-slate-900">
      <div class="absolute inset-0 -z-10 overflow-hidden">
        ...
        <svg class="absolute left-[50%] top-0 h-[64rem] w-[128rem] -translate-x-1/2 stroke-white/5 [mask-image:radial-gradient(64rem_64rem_at_top,white,transparent)]" aria-hidden="true">
          <defs>
            <pattern id="hero-pattern-pricing" width="200" height="200" x="50%" y="-1" patternUnits="userSpaceOnUse">
              <path d="M.5 200V.5H200" fill="none"></path>
            </pattern>
          </defs>
          <rect width="100%" height="100%" stroke-width="0" fill="url(#hero-pattern-pricing)"></rect>
        </svg>
      </div>
      ...
      <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative">
        ...
        >
          ...
          <h1 class="text-4xl sm:text-5xl lg:text-6xl font-bold tracking-tight text-white leading-[1.1]">
            {{ $t('pricing.hero.titlePart1') }}
            <span class="bg-gradient-to-r from-indigo-400 via-purple-400 to-pink-400 bg-clip-text text-transparent">{{ $t('pricing.hero.titleHighlight') }}</span>
            {{ $t('pricing.hero.titlePart2') }}
          </h1>
    ...
    <section id="tiers" class="py-24 scroll-mt-24">
      <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="grid lg:grid-cols-3 gap-8">

          <!-- Bronze -->
          <div
            ref="bronzeReveal.element"
            :class="[
              'opacity-0',
              { 'animate-reveal-up': bronzeReveal.isVisible }
            ]"
          >
            <div class="h-full bg-white rounded-3xl shadow-xl border-2 border-amber-700/30 overflow-hidden hover:shadow-2xl transition-all duration-500 hover:scale-[1.02]">
              <div class="bg-gradient-to-br from-amber-700 to-amber-800 p-8 text-white">
                <span class="inline-block text-xs font-bold uppercase tracking-widest bg-white/20 backdrop-blur-sm px-3 py-1 rounded-full mb-4">{{ $t('pricing.tiers.bronze.badge') }}</span>
                <h3 class="text-2xl font-bold mb-1">{{ $t('pricing.tiers.bronze.name') }}</h3>
                <p class="text-amber-200 text-sm">{{ $t('pricing.tiers.bronze.focus') }}</p>
              </div>
              <div class="p-8">
                <p class="text-gray-600 text-sm leading-relaxed mb-6">{{ $t('pricing.tiers.bronze.audience') }}</p>
                <div class="border-t border-gray-100 pt-6 mb-6">
                  <div class="text-4xl font-extrabold text-gray-900">{{ $t('pricing.tiers.bronze.price') }}</div>
                  <div class="text-sm text-gray-500 mt-1">{{ $t('pricing.tiers.bronze.priceSub') }}</div>
                </div>
                <ul class="space-y-3">
                  <li v-for="(detail, index) in tm('pricing.tiers.bronze.details')" :key="index" class="flex items-start gap-3">
                    <CheckCircleIcon class="h-5 w-5 text-amber-700 mt-0.5 flex-shrink-0" />
                    <span class="text-gray-700 text-sm">{{ rt(detail) }}</span>
                  </li>
                </ul>
              </div>
            </div>
          ...
          <!-- Silver -->
          <div
            ref="silverReveal.element"
            :class="[
              'opacity-0 lg:-mt-4',
              { 'animate-reveal-up': silverReveal.isVisible }
            ]"
          >
            <div class="relative h-full bg-white rounded-3xl shadow-2xl border-2 border-gray-400 overflow-hidden hover:shadow-2xl transition-all duration-500 hover:scale-[1.02]">
              <div class="absolute top-0 right-6 bg-gradient-to-r from-indigo-600 to-purple-600 text-white text-xs font-bold uppercase tracking-wider px-4 py-1.5 rounded-b-lg shadow-lg">{{ $t('pricing.tiers.silver.popular') }}</div>
              <div class="bg-gradient-to-br from-gray-500 to-gray-700 p-8 text-white">
                <span class="inline-block text-xs font-bold uppercase tracking-widest bg-white/20 backdrop-blur-sm px-3 py-1 rounded-full mb-4">{{ $t('pricing.tiers.silver.badge') }}</span>
                <h3 class="text-2xl font-bold mb-1">{{ $t('pricing.tiers.silver.name') }}</h3>
                <p class="text-gray-300 text-sm">{{ $t('pricing.tiers.silver.focus') }}</p>
              </div>
              <div class="p-8">
                <p class="text-gray-600 text-sm leading-relaxed mb-6">{{ $t('pricing.tiers.silver.audience') }}</p>
                <div class="border-t border-gray-100 pt-6 mb-6">
                  <div class="text-4xl font-extrabold text-gray-900">{{ $t('pricing.tiers.silver.price') }}</div>
                  <div class="text-sm text-gray-500 mt-1">{{ $t('pricing.tiers.silver.priceSub') }}</div>
                </div>
                <ul class="space-y-3">
                  <li v-for="(detail, index) in tm('pricing.tiers.silver.details')" :key="index" class="flex items-start gap-3">
                    <CheckCircleIcon class="h-5 w-5 text-gray-600 mt-0.5 flex-shrink-0" />
                    <span class="text-gray-700 text-sm">{{ rt(detail) }}</span>
                  </li>
                </ul>
              </div>
          ...
          <!-- Gold -->
          <div
            ref="goldReveal.element"
            :class="[
              'opacity-0',
              { 'animate-reveal-up': goldReveal.isVisible }
            ]"
          >
            <div class="relative h-full bg-white rounded-3xl shadow-xl border-2 border-yellow-500/30 overflow-hidden hover:shadow-2xl transition-all duration-500 hover:scale-[1.02]">
              <div class="absolute top-0 right-6 bg-yellow-600 text-white text-xs font-bold uppercase tracking-wider px-4 py-1.5 rounded-b-lg shadow-lg">{{ $t('pricing.tiers.gold.enterprise') }}</div>
              <div class="bg-gradient-to-br from-yellow-600 to-amber-600 p-8 text-white">
                <span class="inline-block text-xs font-bold uppercase tracking-widest bg-white/20 backdrop-blur-sm px-3 py-1 rounded-full mb-4">{{ $t('pricing.tiers.gold.badge') }}</span>
                <h3 class="text-2xl font-bold mb-1">{{ $t('pricing.tiers.gold.name') }}</h3>
                <p class="text-yellow-200 text-sm">{{ $t('pricing.tiers.gold.focus') }}</p>
              </div>
              <div class="p-8">
                <p class="text-gray-600 text-sm leading-relaxed mb-6">{{ $t('pricing.tiers.gold.audience') }}</p>
                <div class="border-t border-gray-100 pt-6 mb-6">
                  <div class="text-4xl font-extrabold text-gray-900">{{ $t('pricing.tiers.gold.price') }}</div>
                  <div class="text-sm text-gray-500 mt-1">&nbsp;</div>
                </div>
                <ul class="space-y-3">
                  <li v-for="(detail, index) in tm('pricing.tiers.gold.details')" :key="index" class="flex items-start gap-3">
                    <CheckCircleIcon class="h-5 w-5 text-yellow-600 mt-0.5 flex-shrink-0" />
                    <span class="text-gray-700 text-sm">{{ rt(detail) }}</span>
                  </li>
                </ul>
              </div>
    ...
    <section class="py-16 bg-gradient-to-br from-gray-50 to-indigo-50/30">
      <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        >
          <h2 class="text-2xl font-bold text-gray-900 mb-8">{{ $t('pricing.norms.title') }}</h2>
          <div class="flex flex-wrap justify-center gap-3">
            <span
              v-for="(norm, index) in tm('pricing.norms.items')"
              :key="index"
              :class="[
    ...
    <section class="py-16">
      <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        >
          <h3 class="text-2xl font-bold text-gray-900 mb-3">{{ $t('pricing.crossLink.title') }}</h3>
          <p class="text-gray-600 mb-6 max-w-2xl mx-auto">{{ $t('pricing.crossLink.description') }}</p>
          <NuxtLink
        ...
        <div class="bg-white rounded-3xl shadow-2xl overflow-hidden">
          ...
          <div class="px-8 py-12 sm:px-12 lg:px-16">
            <form action="https://api.web3forms.com/submit" method="POST" class="max-w-xl mx-auto space-y-6">
              ...
              <input type="hidden" name="subject" value="Pricing aanvraag - Helpr.ai Adviesbureaus">
              <input type="hidden" name="from_name" value="Helpr.ai Pricing Page">

              <div>
                <label for="cta-name" class="block text-sm font-medium text-gray-700 mb-2">{{ $t('pricing.cta.form.name') }} *</label>
                <input
                  id="cta-name"
                  name="name"
                  type="text"
                  required
                  :placeholder="$t('pricing.cta.form.namePlaceholder')"
              ...
              <div>
                <label for="cta-email" class="block text-sm font-medium text-gray-700 mb-2">{{ $t('pricing.cta.form.email') }} *</label>
                <input
                  id="cta-email"
                  name="email"
                  required
                  type="email"
                  :placeholder="$t('pricing.cta.form.emailPlaceholder')"
                  class="w-full h-12 rounded-xl border-2 border-gray-200 px-4 text-gray-900 placeholder-gray-500 focus:border-indigo-500 focus:ring-0 transition-colors"
                />
              </div>

              <div>
                <label for="cta-company" class="block text-sm font-medium text-gray-700 mb-2">{{ $t('pricing.cta.form.company') }} *</label>
                <input
                  id="cta-company"
                  name="company"
                  type="text"
                  required
                  :placeholder="$t('pricing.cta.form.companyPlaceholder')"
                  class="w-full h-12 rounded-xl border-2 border-gray-200 px-4 text-gray-900 placeholder-gray-500 focus:border-indigo-500 focus:ring-0 transition-colors"
                />
              </div>

              <div>
                <label for="cta-message" class="block text-sm font-medium text-gray-700 mb-2">{{ $t('pricing.cta.form.message') }}</label>
                <textarea
                  id="cta-message"
                  name="message"
                  :placeholder="$t('pricing.cta.form.messagePlaceholder')"
                  rows="4"
              ...
              >
                {{ $t('pricing.cta.form.submit') }}
              </button>

              <p class="text-xs text-gray-500 text-center">
                {{ $t('pricing.cta.form.privacy') }}
              </p>
            </form>
          </div>
        </div>
...
const bronzeReveal = createRevealElement()
const silverReveal = createRevealElement()
const goldReveal = createRevealElement()
